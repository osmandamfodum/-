	...
	else
	{
		...
		if(!empty($name) && !empty($email) && !empty($phone) && !empty($pass) && !empty($uimage))
		{
			...
			move_uploaded_file($temp_name1,"admin/user/$uimage");
			   if($result){
				   $msg = "<p class='alert alert-success'>تم التسجيل بنجاح</p> ";
			   }
			   else{
				   $error = "<p class='alert alert-warning'>فشل في عملية التسجيل</p> ";
			   }
		}else{
			$error = "<p class='alert alert-warning'>الرجاء اكمال كل الحقول</p>";
		}
	}
                	...
                	<div class="loginbox">
                        <div class="login-right">
							<div class="login-right-wrap">
								<h1>تسجيل</h1>
								<p class="account-subtitle">الوصول لادارة حسابك</p>
								...
								<form method="post" enctype="multipart/form-data">
									...
									<div class="form-check-inline disabled">
									  <label class="form-check-label">
										<input type="radio" class="form-check-input" name="utype" value="builder">معماري
									  </label>
									</div> 
									
									<div class="form-group">
										<label class="col-form-label"><b>صوره المستخدم</b></label>
										<input class="form-control" name="uimage" type="file">
									</div>
									
									<button class="btn btn-success" name="reg" value="Register" type="submit">تسجيل</button>
								...
								<div class="text-center dont-have">لدي حساب مسبقا ؟ <a href="login.php">دخول</a></div>
